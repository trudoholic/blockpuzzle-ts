...
import { type GameInput, type CurrentPiece, type Grid, type PieceId } from "./types";
...
const isCollided = (piece: CurrentPiece, grid: Grid) => {
  ...
  return !!(blocks.length || left.length || right.length || down.length);
};
...
export const isGameOver = (state: State) => {
  return state.grid[1].some((cell) => cell);
